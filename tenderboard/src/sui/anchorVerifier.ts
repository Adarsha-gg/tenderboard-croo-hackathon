...
import type { LiveRunReceipt, SuiReceiptAnchorPayload, TenderBoardConfig } from '../live/types.js';
...
export interface SuiAnchorVerification {
  objectType: 'walrusproof.sui_anchor_verification.v1';
  ok: true;
  runId: string;
  transaction: string;
  network: string;
...
export function parseSuiReceiptAnchorPayload(value: unknown): SuiReceiptAnchorPayload {
  ...
  if (payload.objectType !== 'walrusproof.sui_receipt_anchor_payload.v1') {
...
function hasMatchingAnchorEvent(result: Record<string, unknown>, payload: SuiReceiptAnchorPayload, config: TenderBoardConfig): boolean {
...
function hasMatchingReputationEvent(result: Record<string, unknown>, payload: SuiReceiptAnchorPayload, config: TenderBoardConfig): boolean {
  const events = Array.isArray(result.events) ? result.events : [];
  return events.some((event) => {
    if (!isRecord(event)) return false;
    const type = stringOptional(event.type);
