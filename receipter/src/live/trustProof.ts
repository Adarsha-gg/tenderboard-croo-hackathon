...
export function validateScoutEvidenceIntegrity(workerEvidence: ScoutEvidence | undefined): string[] {
  ...
  if (workerEvidence.schema !== 'receipter.scout_evidence.v1') {
    failures.push('Worker evidence schema is not receipter.scout_evidence.v1.');
  }
  ...
  if (!sourceReceipt) {
    ...
    if (sourceReceipt.schema !== 'receipter.source_receipt.v1') {
      failures.push('Source receipt schema is not receipter.source_receipt.v1.');
    }
