...
export async function buildDemoWorkerDelivery(
  ...
  const deliveryText = [
    `Receipter worker completed: ${receipt.taskTitle}`,
    '',
    'What I did:',
...
export function validateExternalWorkerDelivery(
  ...
  if (payload.objectType !== 'receipter.external_worker_delivery.v1') {
    errors.push('Worker delivery payload objectType must be receipter.external_worker_delivery.v1.');
  }
