import type { LiveRunReceipt, PaymentIntentPlan, SuiWalletTransactionRequest, ReceipterConfig } from '../live/types.js';
...
export interface BuildSuiX402PaymentTransactionRequestInput {
  ...
  config: Pick<ReceipterConfig, 'suiNetwork' | 'suiPackageId' | 'suiReceiptRegistryId'>;
}
...
export function buildSuiX402PaymentTransactionRequestFromIntent(
  ...
  return buildSuiWalletTransactionBase({
    ...
    signerRole: 'hirer',
    description: `Pay ${input.paymentIntent.amountSui} SUI for Receipter worker access.`,
    required: {
      packageId,
