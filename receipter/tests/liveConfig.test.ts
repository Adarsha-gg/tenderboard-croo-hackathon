...
describe('loadReceipterConfig', () => {
  it('returns safe Sui config without leaking unrelated env secrets', () => {
    const config = loadReceipterConfig({
      ...
      MEMWAL_NAMESPACE: 'receipter-test',
    ...
    expect(config.safe.memory).toMatchObject({
      ...
      memwalNamespace: 'receipter-test',
  ...
  it('rejects non-Sui modes', () => {
    expect(() => loadReceipterConfig({ RECEIPTER_MODE: 'live' })).toThrow('Expected sui-dev or sui');
  });

  it('rejects unknown memory backends', () => {
    expect(() => loadReceipterConfig({ MEMORY_BACKEND: 'postgres' })).toThrow('Expected walrus or memwal');
  });
  ...
  it('fails loudly when production MemWal mode is missing credentials', () => {
    ...
    ).toThrow('RECEIPTER_MODE=sui with MEMORY_BACKEND=memwal requires MEMWAL_DELEGATE_KEY, MEMWAL_SERVER_URL');
  });
